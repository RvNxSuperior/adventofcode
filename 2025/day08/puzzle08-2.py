...
while len(Circuits) > 1 and ConnectionCount <= len(DistancesSorted):
...
# getting product of x coordinates of last circuits

print('Calculating product of x coordinates')

ProductXCoordinate = 1

for EndCoordinate in LastConnection[0]:

    ProductXCoordinate *= int(EndCoordinate[1:-1].split(', ')[0])
...
print()
print(f'Circuit product: {ProductXCoordinate}')
